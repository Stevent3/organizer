...
- `GET /fetch?url=` → lädt eine Web-Seite über den Worker (Browser-CORS): `{ok,status,title,ingredients?,text}` – `ingredients` aus schema.org/Recipe (JSON-LD), `text` = Seitentext (max 12k) für die Groq-Extraktion; App: Einkaufsliste → „Zutaten aus Rezept-Link holen“
- **Cron `scheduled()`** → `runChecks()`: (1) Abfahrts-Push (lead = travel+5 min, sonst 30; Fenster ±8/±7 min um departMin; Dedup-Key in KV mit TTL), (2) Morgen-Briefing 06:00–06:25 (optional Groq-formuliert), (3) Smart-Tipps 10:00–10:25 & 14:30–14:55 (Groq entscheidet `{send,title,body}`; auch "kein Tipp" wird als gesendet markiert, max 2/Tag). (4) Abend-Review 21:00–21:25 (To-do-Bilanz, Geburtstage von morgen), (5) Wochen-Vorschau So 19:00–19:25 (Schichten mit Stunden/Verdienst aus `state.work`, Geburtstage, freie Abende). Abfahrts-Push rechnet die Fahrzeit ab `state.home` (aus Mehr → Wetter & Zuhause) über Nominatim + OSRM (gratis, Auto), Cache in KV `geo:<adresse>` (30 Tage, auch „nicht gefunden“) und `route:<von>><nach>` (7 Tage); ohne Zuhause/Ort bleibt es bei 30 Min. Vor allen Checks: `calendarForDay(applyOverrides(rawCal, state.calOverrides), today)` — Termin-Verschiebungen/-Löschungen aus der App gelten auch für Push, und nur heutige Termine zählen (Zeilen mit anderem Datum werden ignoriert).

**Kalender-Zeilenformat (Shortcut→Worker), seit 10.09.2026 mit Datum:** `DD.MM.YYYY | HH:MM | HH:MM | Titel | Ort | Fahrzeit` — Feld 1 (Datum, auch `YYYY-MM-DD` oder 2-stelliges Jahr; Datum und Startzeit dürfen in EINEM Feld stehen wie Kurzbefehle eine Datumsvariable unformatiert einsetzen: `10.09.2026, 08:00 | 10.09.2026, 09:30 | Titel | Ort`; fehlt das Datum, gilt die Zeile als heute = altes Format; `parseStamp` versteht auch „um", „Uhr", Wochentag; 00:00–23:59 = ganztägig; Enddatum an späterem Tag → `endDate`; Folgezeilen ohne Zeitstempel = mehrzeilige Adresse des Termins davor; Rohdaten des letzten POST liegen 7 Tage in KV `calendar_raw`), Ende, Ort und Fahrzeit (erste Ganzzahl wird geparst, z.B. "23 Min.") sind optional. Override-Key: `date|time|text` bei Zeilen mit Datum, sonst `time|text` (App `calKey` und Worker `calKey` identisch halten!). Rezept für den 14-Tage-Kurzbefehl steht in der App unter Mehr → Kurzbefehle (Stand 10.09.2026: Suche VOR der Schleife, ein Filter „Startdatum innerhalb der nächsten 2 Wochen" mit „Alle" – „Mindestens eine" liefert in Kurzbefehlen nichts; Chips aus „Objekt wiederholen": Startdatum | Enddatum | Objekt (=Titel) | Ort | Ist ganztägig; Automation um 23:55, weil Apple „nächste 2 Wochen" ab jetzt rechnet und ein Lauf um 06:00 die ganztägigen Termine des Tages verliert). Titel mit Präfix „Abgesagt:"/„Canceled:" werden verworfen. Fahrzeit-Feature ist gebaut, aber Stevens Shortcut nutzt aktuell 4 Felder (Apple "Wegzeit"-Aktion warf kCLErrorDomain 8 bei nicht geokodierbaren Orten → Wenn-Block wieder entfernt).
...
- Kalender über 14 Tage läuft seit 10.09.2026 (Kurzbefehl umgestellt, Worker + App mit Datum). `legacy/` (v7) zeigt alle Zeilen als heute – bewusst unverändert

...
>
> **Cloud-Sitzung (10.09.2026):** `npm install -g wrangler` (4.130) und `wrangler deploy --dry-run` laufen in der Cloud-Umgebung durch (Bindings env.KV `2f8eb11cc97f48a9a64ef75b35474126`, VAPID_PUBLIC, VAPID_SUBJECT bestätigt). `whoami`/`deploy` brauchen dort `CLOUDFLARE_API_TOKEN` in der Umgebung – war in dieser Sitzung NICHT gesetzt, Worker-Deploy aus der Cloud steht also noch aus (Token in den Umgebungs-Einstellungen von claude.ai/code hinterlegen, dann `wrangler deploy` im Repo-Root; KV nie neu anlegen, Secrets bleiben erhalten).
>
...
>
> **Cloud-Sitzungen (claude.ai/code, Handy):** Das Repo enthält alles Nötige. Frontend-Deploy = `git push`. Worker-Deploy braucht dort `CLOUDFLARE_API_TOKEN` (Vorlage „Edit Cloudflare Workers") als Umgebungsvariable, dann `wrangler deploy` im Repo-Root.
>

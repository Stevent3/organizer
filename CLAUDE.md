...
>
> **Cloud-Sitzung (10.09.2026, zweiter Anlauf):** `CLOUDFLARE_API_TOKEN` ist jetzt in der Umgebung gesetzt, `npm install -g wrangler` (4.130) und `wrangler deploy --dry-run` laufen durch (Bindings env.KV `2f8eb11cc97f48a9a64ef75b35474126`, VAPID_PUBLIC, VAPID_SUBJECT bestätigt). **Blocker ist die Netzwerk-Policy der Cloud-Umgebung, nicht der Token:** Der Egress-Proxy beantwortet CONNECT zu `api.cloudflare.com`, `organizer.steven-ec0.workers.dev`, `api.open-meteo.com` und `api.groq.com` mit 403 (`whoami`/`deploy` melden „fetch failed“). Lösung: Umgebung auf claude.ai/code → Netzwerkzugriff auf „Voller Zugriff“ stellen oder genau diese Hosts freigeben (`sparrow.cloudflare.com` ist nur Telemetrie), dann eine NEUE Sitzung starten (Policy gilt ab Container-Start) und `wrangler deploy` im Repo-Root ausführen; KV nie neu anlegen, Secrets bleiben erhalten. Alternativ `wrangler deploy` auf dem Windows-Rechner. Der Worker-Stand im Repo (Abend-Review-Push, Wetter im Briefing) ist per vitest (`app/src/test/worker.test.ts`, Cron-Durchlauf mit echter Push-Krypto) und `node --check` verifiziert, aber noch NICHT deployt.
>
...
>
> **Cloud-Sitzungen (claude.ai/code, Handy):** Das Repo enthält alles Nötige. Frontend-Deploy = `git push`. Worker-Deploy braucht dort `CLOUDFLARE_API_TOKEN` (Vorlage „Edit Cloudflare Workers") als Umgebungsvariable UND eine Netzwerk-Policy, die `api.cloudflare.com` zulässt (siehe Cloud-Sitzung oben), dann `wrangler deploy` im Repo-Root.
>
